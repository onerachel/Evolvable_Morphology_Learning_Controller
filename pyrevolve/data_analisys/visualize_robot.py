...
async def test_robot_run(robot_file_path: str):
    ...
    if settings.plot_test_robot:
        ...
        ax1.plot([0, 10], [0, 0], linestyle='dashed', label='target direction',linewidth=2.0)   # 0 degree
        # ax1.plot([0, -0.04999999999999998], [0, 0.08660254037844387], linestyle='dashed', label='target direction',linewidth=2.0)  # 120 degree
        # ax1.plot([0, -0.05000000000000004], [0, -0.08660254037844384], linestyle='dashed', label='target direction',linewidth=2.0) # 240 degree
        line10, = ax1.plot([0 for i in range(SIZE)], [0 for i in range(SIZE)], '-', label='robot trajectory', color='#9370db',linewidth=2.0)
